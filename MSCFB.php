...
function decodeMsiString($input)
{
    $k_Msi_Chars = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz._";
    $k_Msi_SpecChar = '!';
    $k_Msi_NumBits = 6;
    $k_Msi_NumChars = 64;
    $k_Msi_CharMask = 63;
    $k_Msi_StartUnicodeChar = 0x3800;
    ...
    while ($i < $inputLen) {
        // wchar_t может быть 8 или 16 бит, 0 - конец строки
        // Считываем wchar (8 или 16 бит) до нулевого символа
        $c = 0;
        
        // Считываем младший байт
        if ($i >= $inputLen) break;
        $byte1 = ord($input[$i++]);
        if ($byte1 == 0) break; // Конец строки для 8-бит
        
        // Проверяем, может быть это 16-битный символ
        if ($i < $inputLen) {
            $byte2 = ord($input[$i]);
            // Если следующий байт ненулевой, вероятно это 16-битный символ
            if ($byte2 != 0) {
                $c = $byte1 | ($byte2 << 8);
                $i++; // Пропускаем второй байт
            } else {
                $c = $byte1;
                $i++; // Пропускаем нулевой байт (конец строки для 16-бит)
                break;
            }
        } else {
            $c = $byte1;
        ...
        if ($c < $k_Msi_StartUnicodeChar || $c > ($k_Msi_StartUnicodeChar + 4160)) {
            return false;
        }
        ...
        if ($c1 <= $k_Msi_NumChars) {
            $result .= $k_Msi_Chars[$c0];
            if ($c1 == $k_Msi_NumChars) {
                break;
            }
            $result .= $k_Msi_Chars[$c1];
        } else {
            $result .= $k_Msi_SpecChar;
        }
    }
...
class MSCFB{
  ...
  private function process_dir_entry($dir_bin){
    ...
    if ($u['type'] === 5)  {
      ...
      if (ord($u['name'][0]) == 5 && ord($u['name'][1] == 0)) { // SummaryInformation
        $u['name16'] = substr($u['name'],2,$u['name_length']-4); 
        $u['name'] = mb_convert_encoding($u['name16'], $this->DE_enc, 'UTF-16LE');
      } else {
        //printf("%x %x %x %x ",ord($u['name'][0]),ord($u['name'][1]), ord($u['name'][2]), ord($u['name'][3]));
        //echo '['.$u['name'].']',decodeMsiString($u['name']), "\n";
        $u['name'] = decodeMsiString($u['name']);
        $u['name16'] = substr($u['name'],0,$u['name_length']-2); // -2 for null haracter terminator
        //$u['name'] = mb_convert_encoding($u['name16'], $this->DE_enc, 'UTF-16LE');	
      }
    }
